...
class DatabaseService {
  ...
  Database? _database;
  String? currentDriverId; // Added to track current driver
  ...
  Future<int> inserirEntrega(Map<String, dynamic> entrega) async {
    ...
    final Map<String, dynamic> entregaCompleta = {
      ...
      'client_name': entrega['clientName'] ?? '',
      'driver_id': entrega['driverId'] ?? '',
    };
  ...
  Future<List<Map<String, dynamic>>> listarEntregasParaApp() async {
    ...
    return entregas.map((entrega) {
      return {
        ...
        'observacoes': entrega['observacoes'],
        'clientName': entrega['client_name'],
      };
    }).toList();
  }
  ...
  // Converter os métodos do Firestore para SQLite
  
  // Stream para obter entregas disponíveis (status pendente)
  Stream<List<Delivery>> getAvailableDeliveries() async* {
    final db = await database;
    ...
    yield results.map((data) {
      DateTime timestamp;
      try {
        timestamp = DateTime.parse(data['data_criacao'] as String);
      } catch (e) {
  ...
  // Obter entregas concluídas pelo motorista atual
  Stream<List<Delivery>> getCompletedDeliveries() async* {
    final db = await database;
    
    // Removendo a condição de driver_id para testes
    // E incluindo tanto "Concluída" quanto "Entregue" como status válidos
    List<Map<String, dynamic>> results = await db.query(
      'entregas',
      where: 'status = ? OR status = ?',
      whereArgs: ['Concluída', 'Entregue'],
      orderBy: 'data_atualizacao DESC'
    );
    
    print('Entregas concluídas encontradas: ${results.length}');
    ...
    yield results.map((data) {
      ...
      return Delivery(
        ...
        deliveryAddress: data['endereco'] ?? '',
        timestamp: timestamp,
        // Adicionando mais campos para depuração
        photoPath: data['foto_assinatura'],
  ...
  // Obter uma entrega específica por ID
  Stream<Delivery?> getDeliveryById(String deliveryId) async* {
    final db = await database;
    List<Map<String, dynamic>> results = await db.query(
      'entregas',
      where: 'codigo = ?',
  ...
  Map<String, dynamic> _converterParaFormatoDaAPI(Map<String, dynamic> dadosBanco) {
    return {
      ...
      'description': dadosBanco['descricao'] ?? 'Entrega #${dadosBanco['codigo'] ?? dadosBanco['id']}',
      ...
      'endereco': dadosBanco['endereco'] ?? 'Endereço da entrega',
      'observacoes': dadosBanco['observacoes'],
      'clientName': dadosBanco['client_name'] ?? 'Cliente',
    };
  }
  ...
  // Set current driver ID (for authentication)
